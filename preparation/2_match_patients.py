...
def costmatrix_matching(patients, controls):

    matches = []
    diffs = []
    for gender in ['male', 'female']:
        patients_sub = {k:v for k, v in patients.items() if v['gender']==gender}
        controls_sub = {k:v for k, v in controls.items() if v['gender']==gender}

        list1 = [v['age'] for v in patients_sub.values()]
        list2 = [v['age'] for v in controls_sub.values()]

        # Ensure list1 is the smaller or equal list
        reverse = False
        if len(list1) > len(list2):
            reverse=True
            list1, list2 = list2, list1

        # Create a cost matrix where the cost is the absolute age difference
        cost_matrix = np.abs(np.subtract.outer(list1, list2))
        ...
        if reverse:
            matches += [(list(patients_sub)[i], list(controls_sub)[j]) for j, i in zip(row_ind, col_ind)]
        else:
            matches += [(list(patients_sub)[i], list(controls_sub)[j]) for i, j in zip(row_ind, col_ind)]

    diffs += [abs(patients[p]['age']- controls[c]['age']) for p, c in matches]

    return list(zip(diffs, matches, strict=True))
...
if __name__ == '__main__':
    ...
    # matches = costmatrix_matching(patients.copy(), controls.copy())
