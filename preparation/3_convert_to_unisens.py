...
# %% create unisens
    ...
    try:
        ...
        print(f'ERROR in {edf_file} for {unisens_folder}')
        raise e
    ...
    if not 'ECG' in u or overwrite:
        ...
        lsb, offset = sleep_utilsx.minmax2lsb(dmin, dmax, pmin, pmax)
    ...
    if not 'EEG' in u or overwrite:
        chs = sleep_utilsx.infer_eeg_channels(all_labels)
    ...
    if not 'EOG' in u or overwrite:
        ...
        chs = sleep_utilsx.infer_eog_channels(all_labels)
        signals, shead, header = read_edf(edf_file, ch_names=chs, digital=True, verbose=False)
        if isinstance(signals, list):
            signals = np.atleast_2d(signals[0])
            chs = chs[0]
    ...
    if not 'EMG' in u or overwrite:
        ...
        chs = sleep_utilsx.infer_emg_channels(all_labels)
    ...
    for file in add_files:
        ...
        # %% add hypnogram
            ...
            hypno = sleep_utilsx.read_hypnogram(file)
